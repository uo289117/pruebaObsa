...
import { renderRanking, ensureIndicadorSelector, descargarTablaActualExcel } from "./ranking.js";
...
let rankingOrden = "alfabetico"; // "alfabetico" | "posicion"
...
function fmt(x) {
  ...
  if (typeof x === "number") return Number.isInteger(x) ? String(x) : x.toFixed(1);
...
const ORDEN_DETERMINANTES = [
  ...
  "Mortalidad asociada alcohol (‰)",
  "Consumo de refrescos y/o comida rápida (%)",
  "Consumo inadecuado refrescos y/o comida rápida (%)",
  ...
  "Población de 25 a 64 años sin educación superior (%)",
  "Desempleo (%)",
  "Clase social baja autopercibida (%)",
  "Personas en clases social VI+VII (%)",
  ...
  "Preocupación por calidad ambiental residencial (%)"
];
...
function getIndicadoresDisponibles(año) {
  ...
  // Añade los que están en el orden ideal y existen realmente en el JSON
  ...
  // Añade el resto que no estaban en tu orden ideal
...
function setState(patch) {
  ...
  if ("rankingOrden" in patch) rankingOrden = patch.rankingOrden;
...
function actualizarVista() {
  ...
  const rankingOrdenWrap = document.getElementById("rankingOrdenWrap");
  const rankingOrdenBtns = document.querySelectorAll('[data-order]');
  ...
  if (modoActual === "ranking") {
   ...
   if (rankingModo === "municipio") {
    ...
    rankingOrdenBtns.forEach(btn => {
      btn.classList.toggle("active", btn.dataset.order === rankingOrden);
    });
...
function repintarMenuIzq(grupo) {
  ...
  // solo con los del grupo (incluyendo títulos), PERO
  // ocultar en el menú: Valoración Global / Resultados / Determinantes
    ...
    if (esTitulo) {
      ...
      div.addEventListener("click", () => {
        ...
        if (modoActual === "ranking") {
          ...
          if (selectorModo) selectorModo.value = modoActual;
        }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll('[data-order]').forEach(btn => {
    btn.addEventListener("click", () => {
      rankingOrden = btn.dataset.order;
      actualizarVista();
    });
});

  document.addEventListener("click", (e) => {
    const btn = e.target.closest('[data-action="btnDescargar"]');
    if (!btn) return;
    const state = getState();
    descargarTablaActualExcel(state);
  });
